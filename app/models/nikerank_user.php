<?php
class NikerankUser extends AppModel{
	var $name = 'NikerankUser';
	...
	public function facebook($facebook_data){
		...
		if($user_data){
			$facebook_data['id'] = $user_data['NikerankUser']['id'];
			$facebook_data['username'] = $user_data['NikerankUser']['username'];//既に利用しているユーザー名で登録 facebookのスクリーンネーム上書き阻止
			$facebook_data['email'] = $user_data['NikerankUser']['email'];//既に利用しているメールアドレスで登録 facebookで登録しているアドレスでの上書き阻止
		}
		$result = $this->save(array("NikerankUser" => $facebook_data));
	}

...
class TsUser extends AppModel{
	...
	public function facebook($facebook_data){
		...
		if($user_data){
			$facebook_data['id'] = $user_data['TsUser']['id'];
			$facebook_data['username'] = $user_data['TsUser']['username'];//既に利用しているユーザー名で登録 facebookのスクリーンネーム上書き阻止
			$facebook_data['email'] = $user_data['TsUser']['email'];//既に利用しているメールアドレスで登録 facebookで登録しているアドレスでの上書き阻止
		}
		$result = $this->save(array("TsUser" => $facebook_data));
	}

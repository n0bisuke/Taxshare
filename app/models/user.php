...
class User extends AppModel{
	...
	public function facebook($facebook_data){
		...
		if($user_data){
			$facebook_data['id'] = $user_data['User']['id'];
			$facebook_data['username'] = $user_data['User']['username'];//既に利用しているユーザー名で登録 facebookのスクリーンネーム上書き阻止
			$facebook_data['email'] = $user_data['User']['email'];//既に利用しているメールアドレスで登録 facebookで登録しているアドレスでの上書き阻止
		}
	...
	function pointup($user_id,$point){
		...
		$conditions = array('User.id' => $user_id);//idが一致するもの
		$contents = array('User.point' => "'{$pointup}'");//
	...
	function pointdwon($user_id,$point){
		...
		$conditions = array('User.id' => $user_id);//idが一致するもの
		$contents = array('User.point' => "'{$pointdwon}'");//
		$this->updateAll($contents,$conditions);
	}
